...
单页面 HTML · 零依赖 · 零后端 · 方便使用

> 🧭 **项目定位**：面向写作、演讲与内容创作场景，将“查出处”和“改表达”拆成两条明确工作流，降低真实引用与原创表达混用的风险。
>
> 🧩 **工程取向**：单文件、无账号、无后端，适合直接打开使用与自行部署；引用库保存在当前浏览器。
...
> ⚠️ **使用边界**：置信度来自模型判断，不等同于外部数据库或原始文献验证。用于出版、学术、法律或正式演讲前，请回到作品原文或可靠版本进行二次核验。
...
> 🔐 **隐私边界**：本地存储不代表内容不离开设备；发起查询时，输入会发送至 DeepSeek API。请避免提交敏感或受保密约束的文本，并在共享设备上及时清理本地数据与 API Key。
